...
const BuilderFinallyNode: FC<BuilderFinallyNodeProps> = ({ element }) => {
    ...
    <path
      ...
      d={
        nodeCenter + taskIndex * verticalHeight === height / 2
          ? straightPath(
              new Point(leftPadding, height / 2),
  ...
  return (
    <g data-test="builder-finally-node">
      <rect
        className="opp-builder-finally-node"
        width={width}
        height={height}
        rx="20"
        ry="20"
      ...
      {finallyLoadingTasks.map((fld, i) => (
        <g
          key={fld.name}
          data-test={`finally-loading-task-list-node ${fld.name}`}
        >
      ...
      {finallyInvalidListTasks.map((ivl, i) => (
        <g
          key={ivl.name}
          data-test={`finally-invalid-task-list-node ${ivl.name}`}
        >
          <IntegralShape
            taskIndex={i + finallyTasks.length + finallyLoadingTasks.length}
          />
          <g
            transform={`translate(${leftPadding},
              ${
          ...
          >
            ...
            <ErrorNodeDecorator
              ...
              y={BUILDER_NODE_DECORATOR_RADIUS / 4}
              errorStr={t('Task does not exist')}
            />
          </g>
        </g>
      ))}
