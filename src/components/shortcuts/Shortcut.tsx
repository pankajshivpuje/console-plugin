  ...
  return (
    <tr>
      <td className="ocs-shortcut__cell">
        ...
        {shift && (
          <ShortcutCommand data-test-id="shift-button">Shift</ShortcutCommand>
        )}
        {isMac && ctrlCmd && (
          <ShortcutCommand data-test-id="cmd-button">⌘ Cmd</ShortcutCommand>
        )}
        {hover && (
        ...
        {keyName && (
          <ShortcutCommand data-test-id={`${keyName}-button`}>
            {keyName.length === 1
              ? keyName.toUpperCase()
              : _.upperFirst(keyName.toLowerCase())}
          </ShortcutCommand>
        )}
        {drag && (
