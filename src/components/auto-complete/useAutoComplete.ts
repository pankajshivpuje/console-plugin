...
const useOpenMenuKeyWatcher = (): MenuKeyWatcherHook => {
  ...
  const shouldOpen = useCallback<ShouldOpenCallback>(
    (event) => {
      ...
      if (
        lastCharacter === PARAM_REFERENCE_CHARS[0] &&
        key === PARAM_REFERENCE_CHARS[1]
      ) {
        // Manual open
        setOpen(true);
        return true;
    ...
    filterMappings.current = options.reduce(
      (acc: FilterMap, option: string, idx: number) => {
        const optionPartMap = option
          .split('.')
          .reduce((optionAcc, optionPart: string) => {
            return {
              ...optionAcc,
    ...
    (newFilterValue: string) => {
      ...
      const newOptions: string[] = _.uniq(
        Object.keys(filterMappings.current)
          .filter((value: string) =>
            value.toLowerCase().startsWith(newFilterValue.toLowerCase()),
          )
  ...
  const onKeyCallback = useCallback(
    (e) => {
      ...
      if (isOpen) {
        ...
        cursorPosition.current = [
          nodeRef.current.selectionStart,
          nodeRef.current.selectionEnd,
        ];
  ...
  const insertAutoComplete = useCallback(
    (newValue: string) => {
      ...
      const leftCapturePoint =
        cursorPosition.current[0] - PARAM_REFERENCE.length;
      const prefix =
        leftCapturePoint >= 0
          ? nodeRef.current.value.substr(
              leftCapturePoint,
              PARAM_REFERENCE.length,
            )
          : null;
      ...
      const cursorEndPoint: number =
        cursorPosition.current[0] + insertValue.length;
      setTimeout(() => {
