...
export const getFormData = (
  ...
  const sanitizedListTasks: PipelineBuilderTaskBase[] = listTasks.map(
    (listTask) => {
...
export const getValidatedFormAndYaml = (
  ...
  const paths: string[] =
    error?.inner?.map((err) => err.path).filter((path) => !!path) || [];
  ...
  const validatedFormData: PipelineBuilderFormValues = getFormData(
    formData,
    validatedYamlData,
  );
...
export const sanitizeToForm = async (
  ...
  const newFormData: PipelineBuilderFormValues = getFormData(
    formData,
    yamlPipeline,
  );
  try {
    ...
    const safeReturn =
      (await handleSanitizeToFormError(newFormData, error, yamlPipeline)) ||
      formData;
    return () => safeReturn;
  }
