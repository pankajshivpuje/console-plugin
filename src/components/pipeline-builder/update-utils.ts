...
const addListNode: UpdateOperationAction<UpdateOperationAddData> = (
  taskGrouping,
  data,
) => {
  ...
  switch (direction) {
      ...
      return {
        ...
        listTasks: [
          ...listTasks.map((listTask) =>
            mapBeRelated<PipelineBuilderListTask>(
              newTaskName,
              relatedTaskName,
              listTask,
            ),
    ...
    case AddNodeDirection.AFTER:
      return {
        ...taskGrouping,
        tasks: tasks.map((pipelineTask) =>
          mapReplaceRelatedInOthers<PipelineTask>(
            newTaskName,
            relatedTaskName,
            pipelineTask,
          ),
    ...
    case AddNodeDirection.PARALLEL:
      return {
        ...taskGrouping,
        tasks: tasks.map((pipelineTask) =>
          mapAddRelatedToOthers<PipelineTask>(
            newTaskName,
            relatedTaskName,
            pipelineTask,
          ),
        ...
        listTasks: [
          ...listTasks.map((listTask) =>
            mapAddRelatedToOthers<PipelineBuilderListTask>(
              newTaskName,
              relatedTaskName,
              listTask,
            ),
...
> = (taskGrouping, data) => {
  ...
  const { tasks, listTasks, finallyTasks, finallyListTasks, loadingTasks } =
    taskGrouping;
  const usedNames = getTaskNames([...tasks, ...finallyTasks]);
  const newLoadingTask = convertResourceToLoadingTask(
    usedNames,
    resource,
    isFinallyTask,
    runAfter,
  ...
  return {
    ...
    tasks: [
      ...tasks
        .filter((n) => n.name !== name)
        .map((pipelineTask) =>
          mapReplaceRelatedInOthers(newLoadingTask.name, name, pipelineTask),
        ),
    ],
    finallyTasks: [
      ...finallyTasks
    ...
    finallyListTasks: finallyListTasks
      .filter((n) => n.name !== name && isFinallyTask)
      .map((listTask) =>
        mapReplaceRelatedInOthers(newLoadingTask.name, name, listTask),
      ),
  };
};
...
>(
  ...
  const removalTask = updateAndRemoveTasks.find(
    (task) => task.name === removalTaskName,
  );
  if (!removalTask) {
    return {
  ...
  return {
    updateOnlyTasks: updateOnlyTasks.map((task) =>
      mapStitchReplaceInOthers<UT>(removalTask, task),
    ),
...
const deleteListTask: UpdateOperationAction<
  UpdateOperationDeleteListTaskData
> = (taskGrouping, data) => {
...
export const removeTask: UpdateOperationAction<
  UpdateOperationRemoveTaskData
> = (taskGrouping, data) => {
  const { taskName } = data;
  const { tasks, listTasks, finallyTasks, finallyListTasks } = taskGrouping;
  ...
  const { updateAndRemoveTasks: finallyUpdateAndRemoveTasks } =
    removeAndUpdateTasks<PipelineTask, PipelineBuilderListTask>(
      taskName,
      finallyTasks,
      finallyListTasks,
    );
...
const renameTask: UpdateOperationAction<UpdateOperationRenameTaskData> = (
  taskGrouping,
  data,
) => {
  ...
  const getUpdatedTasks = (updateTasks: PipelineTask[]): PipelineTask[] =>
    updateTasks.map((pipelineTask) => {
      if (pipelineTask.name !== preChangePipelineTask.name) {
        ...
        return mapReplaceRelatedInOthers(
          newName,
          preChangePipelineTask.name,
          pipelineTask,
        );
      }
...
const addFinallyListTask: UpdateOperationAction<
  UpdateOperationConvertToFinallyTaskData
> = (taskGrouping, data) => {
  ...
  switch (type) {
    ...
    case UpdateOperationType.DELETE_LIST_TASK:
      return deleteListTask(
        taskGrouping,
        data as UpdateOperationDeleteListTaskData,
      );
    ...
    case UpdateOperationType.ADD_FINALLY_LIST_TASK:
      return addFinallyListTask(
        taskGrouping,
        data as UpdateOperationConvertToFinallyTaskData,
      );
