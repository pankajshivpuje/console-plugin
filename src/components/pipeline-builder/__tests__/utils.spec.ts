...
describe('findTaskFromFormikData / findTask', () => {
  ...
  it('should decompose formik state & handle nulls', () => {
    ...
    expect(
      findTaskFromFormikData(formValues, {
        name: 'test',
        taskRef: { name: 'test' },
      }),
    ).toBe(null);
...
describe('safeName', () => {
  ...
  it('should return a random 3 digit suffix if already in use', () => {
    expect(safeName(['in-use'], 'in-use')).toMatch(
      new RegExp('^in-use-[a-z0-9]{3}$'),
    );
  });
});
...
describe('convertResourceToTask', () => {
  describe('task with param variety', () => {
    ...
    it('should find the same amount of params that was given', () => {
      expect(result.params).toHaveLength(
        externalTaskWithVarietyParams.spec.params.length,
      );
    });
...
describe('runAfter Manipulation Utils', () => {
  describe('mapReplaceRelatedInOthers', () => {
    it('should handle nulls', () => {
      ...
      expect(
        mapReplaceRelatedInOthers<PipelineBuilderTaskBase>(null, null, task1),
      ).toBe(task1);
      const task2: PipelineBuilderTaskBase = {
        name: 'example-task',
        runAfter: [],
      };
      expect(
        mapReplaceRelatedInOthers<PipelineBuilderTaskBase>(null, null, task2),
      ).toBe(task2);
      const task3: PipelineBuilderTaskBase = {
        name: 'example-task',
        runAfter: ['first'],
      };
      expect(
        mapReplaceRelatedInOthers<PipelineBuilderTaskBase>(null, null, task3),
      ).toBe(task3);
    });
    ...
    it('should do nothing if there are no runAfters', () => {
      ...
      expect(
        mapReplaceRelatedInOthers<PipelineBuilderTaskBase>(
          'test',
          'related-name',
          task,
        ),
    ...
    it('should replace a related runAfter with a new runAfter', () => {
      ...
      expect(result.runAfter).not.toEqual(
        expect.arrayContaining(['replace-me']),
      );
  ...
  describe('mapRemoveRelatedInOthers', () => {
    it('should handle nulls', () => {
      ...
      const task1: PipelineBuilderTaskBase = { name: 'example-task' };
      expect(
        mapRemoveRelatedInOthers<PipelineBuilderTaskBase>(null, task1),
      ).toBe(task1);
      const task2: PipelineBuilderTaskBase = {
        name: 'example-task',
        runAfter: [],
      ...
      expect(
        mapRemoveRelatedInOthers<PipelineBuilderTaskBase>(null, task2),
      ).toBe(task2);
      const task3: PipelineBuilderTaskBase = {
        name: 'example-task',
        runAfter: ['first'],
      ...
      expect(
        mapRemoveRelatedInOthers<PipelineBuilderTaskBase>(null, task3),
      ).toBe(task3);
    });
    ...
    it('should do nothing if there are no runAfters', () => {
      const task: PipelineBuilderTaskBase = { name: 'example-task' };
      expect(
        mapRemoveRelatedInOthers<PipelineBuilderTaskBase>('test', task),
      ).toBe(task);
    });
    ...
    it('should do nothing if the runAfters do not include the target', () => {
      ...
      expect(
        mapRemoveRelatedInOthers<PipelineBuilderTaskBase>('remove-me', task),
      ).toBe(task);
    });
    ...
    it('should remove a related runAfter if it matches', () => {
      ...
      const result = mapRemoveRelatedInOthers<PipelineBuilderTaskBase>(
        'remove-me',
        task,
      );
      expect(result.runAfter).not.toEqual(
        expect.arrayContaining(['remove-me']),
      );
  ...
  describe('mapStitchReplaceInOthers', () => {
    it('should handle nulls', () => {
      ...
      const task1: PipelineBuilderTaskBase = { name: 'example-task' };
      expect(
        mapStitchReplaceInOthers<PipelineBuilderTaskBase>(null, task1),
      ).toBe(task1);
      expect(
        mapStitchReplaceInOthers<PipelineBuilderTaskBase>(task1, null),
      ).toBe(null);
      const task2: PipelineBuilderTaskBase = {
        name: 'example-task',
        runAfter: [],
      ...
      expect(
        mapStitchReplaceInOthers<PipelineBuilderTaskBase>(null, task2),
      ).toBe(task2);
      expect(
        mapStitchReplaceInOthers<PipelineBuilderTaskBase>(task2, null),
      ).toBe(null);
      const task3: PipelineBuilderTaskBase = {
        name: 'example-task',
        runAfter: ['first'],
      ...
      expect(
        mapStitchReplaceInOthers<PipelineBuilderTaskBase>(null, task3),
      ).toBe(task3);
      expect(
        mapStitchReplaceInOthers<PipelineBuilderTaskBase>(task3, null),
      ).toBe(null);
    });

    it('should just remove if there are no runAfters in removalTask', () => {
      const target: PipelineBuilderTaskBase = { name: 'target' };
      const source: PipelineBuilderTaskBase = {
        name: 'source',
        runAfter: ['target'],
      };
      expect(
        mapStitchReplaceInOthers<PipelineBuilderTaskBase>(target, source),
      ).toEqual({
        name: 'source',
        runAfter: [],
      });
    ...
    it('should remove the task and consume the runAfters of the removalTask', () => {
      const target: PipelineBuilderTaskBase = {
        name: 'target',
        runAfter: ['first'],
      };
      const source: PipelineBuilderTaskBase = {
    ...
    it('should remove the task and combine runAfters with the removalTask (dupe runAfter)', () => {
      ...
      const source: PipelineBuilderTaskBase = {
        name: 'source',
        runAfter: ['target', 'first'],
      };
      const result = mapStitchReplaceInOthers<PipelineBuilderTaskBase>(
        target,
        source,
      );
  ...
  describe('mapBeRelated', () => {
    it('should handle nulls', () => {
      ...
      expect(mapBeRelated<PipelineBuilderTaskBase>(null, null, task)).toBe(
        task,
      );
      expect(mapBeRelated<PipelineBuilderTaskBase>('test', null, task)).toBe(
        task,
      );
      expect(mapBeRelated<PipelineBuilderTaskBase>(null, 'test', task)).toBe(
        task,
      );
    });
    ...
    it('should add runAfter if matching name is provided', () => {
      const task: PipelineBuilderTaskBase = { name: 'task-name' };
      expect(
        mapBeRelated<PipelineBuilderTaskBase>('value', 'task-name', task),
      ).toEqual({
        name: 'task-name',
        runAfter: ['value'],
      });
  ...
  describe('mapAddRelatedToOthers', () => {
    it('should handle nulls', () => {
      expect(
        mapAddRelatedToOthers<PipelineBuilderTaskBase>(null, null, null),
      ).toBe(null);
      const task: PipelineBuilderTaskBase = { name: 'task-name' };
      expect(
        mapAddRelatedToOthers<PipelineBuilderTaskBase>(null, null, task),
      ).toBe(task);
      expect(
        mapAddRelatedToOthers<PipelineBuilderTaskBase>('test', null, task),
      ).toBe(task);
      expect(
        mapAddRelatedToOthers<PipelineBuilderTaskBase>(null, 'test', task),
      ).toBe(task);
    });
    ...
    it('should do nothing if there are no runAfters', () => {
      const task: PipelineBuilderTaskBase = { name: 'task-name' };
      expect(
        mapAddRelatedToOthers<PipelineBuilderTaskBase>('test', 'test', task),
      ).toBe(task);
      expect(
        mapAddRelatedToOthers<PipelineBuilderTaskBase>(
          'task-name',
          'task-name',
          task,
        ),
  ...
  it('should drop unlinked resources', () => {
    const task: PipelineTask = {
      ...
      resources: {
        ...
        outputs: [
          {
            name: 'required-out-resource',
            resource: 'pipeline-image-resource',
          },
...
describe('getBuilderTasksErrorGroup', () => {
  ...
  it('should not error out if given nothing', () => {
    ...
    expect(getBuilderTasksErrorGroup(undefined, undefined)).toEqual(
      defaultResponse,
    );
  ...
  it('should ignore other status values', () => {
    expect(
      getBuilderTasksErrorGroup({}, { submitError: 'some other error' }),
    ).toEqual(defaultResponse);
  });
