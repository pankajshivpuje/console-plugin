...
export const ExploreType: FC<ExploreTypeProps> = (props) => {
  ...
  const getDrilldownPath = (name: string): string[] => {
    ...
    return _.has(child, 'properties') || _.has(child, 'items.properties')
      ? path
      : null;
  };
  ...
  const getTypeForRef = (ref: string): string =>
    _.get(allDefinitions, [ref, 'format']) ||
    _.get(allDefinitions, [ref, 'type']);
            ...
            {_.map(
              ...
              (definition: SwaggerDefinition, name: string) => {
                ...
                const definitionType =
                  definition.type || getTypeForRef(getRef(definition));
                  ...
                  <li key={name} className="co-resource-sidebar-item">
                    ...
                    {path && (
                      <Button
                        type="button"
                        onClick={(e) =>
                          drilldown(e, name, definition.description, path)
                        }
                        isInline
                        variant="link"
                      >
